...
if (!token) {
  throw new Error("Define BOT_TOKEN in your .env file");
}
...
communities[groupId] = {
  ...
  voters: {
    [userId]: {
      approved: boolean,
      weight: number | null,
      processed: boolean, // true if admin already decided on their join request
      username: string,
...
waitingMyVoteSelection[userId] = true
  // tracks if user is in /myvote interactive selection
...
function getOrInitVoterRecord(comm, fromUser) {
  ...
  if (!comm.voters[uid]) {
    comm.voters[uid] = {
      approved: false,
      weight: null,
      processed: false, // not decided yet
...
bot.command("join", async (ctx) => {
  ...
  for (const gid of communityIds) {
    ...
    // se já foi processado antes, não notifica admin de novo
...
// approve_<groupId>_<userId>_<weight>
// custom_<groupId>_<userId>
// reject_<groupId>_<userId>
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// Voting in DM: dmvote_<groupId>_<proposalId>_<optionIdx>
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// dmresults_... exists in older code, but since we no longer show live
// results during voting and we removed the "View Results" button,
// we could keep it restricted to after-close only, or remove it.
// We'll keep a safe version in case old callbacks fire.
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// /setweight  (DM admin only)
// This is now an INTERACTIVE FLOW with steps.
// 1. /setweight → choose community by name (sw_comm_<groupId>)
// 2. choose user in that community (sw_user_<userId>)
// 3. ask new weight (ASK_WEIGHT)
// 4. ask reason (ASK_REASON)
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// on("message"): handles 3 flows
// - pendingSetWeight (ASK_WEIGHT / ASK_REASON)
// - pendingCustomWeight (Approve custom during first approval)
// - draftProposal (/new flow)
// ---------------------------------------------------------------------
bot.on("message", async (ctx) => {
  ...
  // (A) Flow: /setweight steps ASK_WEIGHT → ASK_REASON
  if (privateChat && pendingSetWeight[fromId]) {
    ...
    if (flow.step === "ASK_REASON" && isText) {
      ...
      voter.processed = true; // stays processed
  ...
  // (B) Flow: pendingCustomWeight (first-time custom approval)
  if (isText && pendingCustomWeight[fromId]) {
    ...
    if (!comm || !isAdmin(comm, fromId)) {
      ...
      if (!isNaN(wNum) && wNum > 0) {
        const voter = comm.voters[targetUserId];
        if (voter) {
          // block repeated action if already processed
          if (voter.processed === true) {
            await ctx.reply(
          ...
          await ctx.reply(
            "User not found in that community."
          );
        }
        delete pendingCustomWeight[fromId];
        return;
...
// ---------------------------------------------------------------------
// publish_<groupId>  (finalizing /new)
// ---------------------------------------------------------------------
bot.callbackQuery(/publish_(-?\d+)/, async (ctx) => {
  ...
  // 3. DM all approved voters with vote keyboard
  for (const [uidStr, voter] of Object.entries(comm.voters)) {
    const uid = Number(uidStr);
    ...
    try {
      ...
      // user never /start'd me, can't DM them. That's OK.
    }
  }
